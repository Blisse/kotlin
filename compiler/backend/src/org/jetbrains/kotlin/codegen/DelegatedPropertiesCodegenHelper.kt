...
class DelegatedPropertiesCodegenHelper(private val state: GenerationState) {
    ...
    private fun isDelegatedPropertyMetadataRequired(operatorCall: ResolvedCall<FunctionDescriptor>): Boolean {
        ...
        return isDelegatedPropertyMetadataRequired(calleeDescriptor).also {
    ...
    private fun isDelegatedPropertyMetadataRequired(calleeDescriptor: FunctionDescriptor): Boolean {
        if (!calleeDescriptor.isInline) {
            return true
        }

        if (calleeDescriptor is DescriptorWithContainerSource) {
            return isMetadataParameterUsedInCompiledMethodBody(
                getMetadataParameterIndex(calleeDescriptor),
                InlineCodegen.createSpecialInlineMethodNodeFromBinaries(calleeDescriptor, state)
            )
        }

        val metadataParameter = calleeDescriptor.valueParameters[1]
        if (bindingContext[BindingContext.UNUSED_DELEGATED_PROPERTY_OPERATOR_PARAMETER, metadataParameter] == true) {
            return false
        }
    ...
    private fun isMetadataParameterUsedInCompiledMethodBody(metadataParameterIndex: Int, methodNode: MethodNode): Boolean =
        methodNode.instructions.toArray().any {
            it is VarInsnNode && it.opcode == Opcodes.ALOAD && it.`var` == metadataParameterIndex
        }
